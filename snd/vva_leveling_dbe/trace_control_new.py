...
def ramp_output(pna, vdi, start_power, stop_power, num_points, f_start_ghz, f_stop_ghz=None, sweep_type='LIN', start_address=0, atten_file="VVA_Attenuation_Data.xlsx", max_iters=4, tolerance=0.15):
    ...
    return target_voltages
# ============================================================================
# CHANNEL INJECTION (CH1_CALREG)
# ============================================================================
def inject_active_channel_error_terms(pna, vdi, num_points, f_start_ghz, f_stop_ghz, sweep_type='LIN', start_address=0, prn_file="VNAX 3437 Test Port Power.prn"):
    """
    Overwrites the 'a1' and 'b2' error terms inside the VNA's active channel register
    using the CH1_CALREG
    """
    print("\n--- Executing Dual-Trace Channel Register Injection (a1 & b2) ---")
    import math 
    import time
    ...
    try: 
        pna.write(f'CALC1:PAR:DEL "{a1_trace}"')
        pna.write(f'CALC1:PAR:DEL "{b2_trace}"')
    except: pass
    
    # 0. Set up BOTH traces for raw capture
    print("Initializing a1 and b2 measurement traces...")
    
    # Hardware receivers: R1,1 (a1) and B,1 (b2)
    pna.write(f'CALC1:PAR:DEF:EXT "{a1_trace}", "R1,1"')
    pna.write(f'CALC1:PAR:DEF:EXT "{b2_trace}", "B,1"')
    
    pna.write("DISP:WIND1:STATE ON")
    
    # Feed to TRAC5 and TRAC6 so we don't overwrite 2-port S-parameters from Cal
    pna.write(f'DISP:WIND1:TRAC5:FEED "{a1_trace}"')
    pna.write(f'DISP:WIND1:TRAC6:FEED "{b2_trace}"')
    
    pna.write(f'CALC1:PAR:SEL "{a1_trace}"')
    pna.write("CALC1:FORM MLOG")
    pna.write(f'CALC1:PAR:SEL "{b2_trace}"')
    pna.write("CALC1:FORM MLOG")
    
    pna.write("SENS1:CORR:STAT OFF") 
    ...
    # 2. Capture the Raw Baseline Sweeps
    print("Capturing raw a1 and b2 baseline sweeps...")
    ...
    pna.write(f'CALC1:PAR:SEL "{a1_trace}"')
    raw_a1 = np.array(pna.query_ascii_values("CALC1:DATA? FDATA"))
    
    pna.write(f'CALC1:PAR:SEL "{b2_trace}"')
    raw_b2 = np.array(pna.query_ascii_values("CALC1:DATA? FDATA"))
    
    # 3. Parse PRN file locally
    print(f"Reading PRN Target Table from your computer: {prn_file}")
    prn_freqs = []
    prn_powers = []
    ...
    # 4. Calculate Independent Error Terms
    print("Calculating a1 and b2 error terms...")
    err_db_a1 = raw_a1 - interp_target_pwr
    err_db_b2 = raw_b2 - interp_target_pwr
    
    linear_mag_a1 = 10 ** (err_db_a1 / 20.0)
    linear_mag_b2 = 10 ** (err_db_b2 / 20.0)
    
    err_array_a1 = np.zeros(num_points * 2)
    err_array_a1[0::2] = linear_mag_a1
    err_array_b2 = np.zeros(num_points * 2)
    err_array_b2[0::2] = linear_mag_b2
    
    str_a1 = ",".join([f"{val:.6f}" for val in err_array_a1])
    str_b2 = ",".join([f"{val:.6f}" for val in err_array_b2])
    
    # 5. Piggyback Injection using working CH1_CALREG workaround
    print("Injecting array into VNA CalSet...")
    cset_name = "CH1_CALREG"
    
    #set to 0 and doesnt change active screen settings back to cal state??? maybe that
    pna.write(f'SENS1:CORR:CSET:ACTivate "{cset_name}", 1')
    
    # Pushing the data into the hardware register
    pna.write(f'SENS1:CORR:CSET:ETERm "ResponseTracking(a1)", {str_a1}')
    pna.write(f'SENS1:CORR:CSET:ETERm "ResponseTracking(b2)", {str_b2}')
    pna.query("*OPC?")
    ...
    pna.write(f'CALC1:PAR:SEL "{a1_trace}"')
    pna.write('CALC1:CORR:TYPE "Response(a1)"')
    
    pna.write(f'CALC1:PAR:SEL "{b2_trace}"')
    pna.write('CALC1:CORR:TYPE "Response(b2)"')
    pna.query("*OPC?")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ==========================================
        # 2. Dual-Trace Error Injection
        # ==========================================
        run_spc = input("\nRun a1/b2 error injection into active Channel Register? (y/n) [Default n]: ").strip().lower()
        
        if run_spc == 'y':
            prn_path = "VNAX 3437 Test Port Power.prn"
            
            inject_active_channel_error_terms(
                pna=pna, 
                vdi=vdi, 
            ...
            input("\n[PAUSED]  Both a1 and b2 should match the Test Port Power.\nPress Enter to continue to the operations menu...")
        else:
            print("\nSkipping error injection. Proceeding with the VNA's current correction state.")
